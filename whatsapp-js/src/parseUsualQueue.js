export function parseMessage(message) {
  ...
  const playersWithIndex = playersStr.split("\n");
  const players = playersWithIndex.map((line) => removeIndex(line));

  const waitlistRegex = /(?<=Waitlist\n)([\d\D]*)(?=\nPlease)/; // Regex to extract the waitlist
  const waitlistStr = waitlistRegex.exec(message)[0];
  const waitlistWithIndex = waitlistStr.split("\n").slice(0, -1);
  const waitlist = waitlistWithIndex.map((line) => removeIndex(line));
